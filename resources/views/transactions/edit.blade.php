...
@section('content')
    @include('users.partials.header', ['title' => __('Edit Transaction')])
                        ...
                        <form method="post" action="{{ route('transactions.update', $transaction) }}" autocomplete="off">
                            ...
                            <div class="pl-lg-4">
                                <div class="form-group{{ $errors->has('name') ? ' has-danger' : '' }}">
                                    ...
                                    <input type="text" name="name" id="input-name" class="form-control form-control-alternative{{ $errors->has('name') ? ' is-invalid' : '' }}" placeholder="{{ __('Name') }}" value="{{ old('name', $transaction->name) }}" required autofocus>
